...
router.post('/', requireAdmin, (req, res) => {
  const { customer_id, name, description, budget_monthly, status, start_date, end_date, platform } = req.body;
  ...
  const result = db.prepare(`
    INSERT INTO campaigns (customer_id, name, description, budget_monthly, status, start_date, end_date, platform)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    customer_id, name,
    description || null,
...
router.put('/:id', requireAdmin, (req, res) => {
  ...
  const { name, description, budget_monthly, status, start_date, end_date, platform } = req.body;

  db.prepare(`
    UPDATE campaigns SET
      name = ?, description = ?, budget_monthly = ?, status = ?,
      start_date = ?, end_date = ?, platform = ?, updated_at = datetime('now')
    WHERE id = ?
  `).run(
    name ?? campaign.name,
