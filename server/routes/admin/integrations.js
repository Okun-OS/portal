...
// POST /api/admin/integrations/google/sync
router.post('/google/sync', requireAdmin, async (req, res) => {
  const { campaign_id } = req.body;
  try {
    if (campaign_id) {
      const result = await googleAds.syncCampaign(campaign_id);
      res.json({ success: true, results: [{ campaign_id, ...result, status: 'ok' }] });
    } else {
      const results = await googleAds.syncAllCampaigns();
      res.json({ success: true, results });
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.get('/google/status', requireAdmin, (req, res) => {
  const db = require('../../db');
  const campaigns = db.prepare(`
    SELECT id, name, meta_campaign_id, google_campaign_id, status
  ...
  const configured = campaigns.filter(c => c.google_campaign_id);
  res.json({
    total: campaigns.length,
    google_configured: configured.length,
    api_ready: !!(process.env.GOOGLE_ADS_DEVELOPER_TOKEN && process.env.GOOGLE_ADS_REFRESH_TOKEN && process.env.GOOGLE_ADS_CUSTOMER_ID),
    campaigns: campaigns.map(c => ({
      id: c.id,
      name: c.name,
      meta_campaign_id: c.meta_campaign_id || null,
